...
{
    public class CacheRemoverRequest<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>, ICacheRemoveRequest
    {
        private readonly IDistributedCache _distributedCache;

        public CacheRemoverRequest(IDistributedCache distributedCache)
        {
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            if (request.CacheKey != null) await _distributedCache.RemoveAsync(request.CacheKey, cancellationToken);
