...
namespace CoreApplication.Pipelines.Caching
{
    public class CachingBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>, ICachableRequest
    {
        ...
        public CachingBehavior(CacheSettings cacheSettings, ILogger<CachingBehavior<TRequest, TResponse>> logger, IDistributedCache cache, IConfiguration configuration)
        {
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            {
                response = await getResponseAndToAddCatch(request, next, cancellationToken);
            }

            return response;
        }

        private async Task<TResponse> getResponseAndToAddCatch(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            await _cache.SetAsync(request.CacheKey, serializedata, distributedCacheEntryOptions, cancellationToken);
        ...
        private async Task addCacheKeyToGroup(TRequest request, TimeSpan slidingExpiration, CancellationToken cancellationToken)
        {
            ...
            if (cacheGroupCacheSlidingExpirationCache != null)
                cacheGroupCacheSlidingExpirationValue = Convert.ToInt32(Encoding.UTF8.GetString(cacheGroupCacheSlidingExpirationCache));
