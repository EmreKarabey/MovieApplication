...
{
    public class _SavedCategoryFilteringComponent : _BaseComponent
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public _SavedCategoryFilteringComponent(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string? CategoryName, int pageIndex = 0)
        {
            ...
            var response = await client.GetAsync($"api/SavedMovie/GetSavedCategory?PageIndex={pageIndex}&PageSize=10&UserId={GetUserId()}&CategoryName={CategoryName}");

            if (!string.IsNullOrEmpty(CategoryName)) response = await client.GetAsync($"api/SavedMovie/GetSavedCategory?PageIndex={pageIndex}&PageSize=10&UserId={GetUserId()}&CategoryName={CategoryName}");
